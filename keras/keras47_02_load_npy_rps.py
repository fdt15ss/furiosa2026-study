...
x_train = np.load(npy_path + 'keras46_rps_x_train.npy')
y_train = np.load(npy_path + 'keras46_rps_y_train.npy')
x_test = np.load(npy_path + 'keras46_rps_x_test.npy')
y_test = np.load(npy_path + 'keras46_rps_y_test.npy')
...
save_path = './_save/rps/' + datetime.now().strftime('%Y%m%d_%H%M%S')+ '_{epoch:04d}-{val_loss:.4f}.keras'
mcp = ModelCheckpoint(
    filepath = save_path,
    monitor = 'val_loss',
    mode = 'auto',
...
my_util.record_model_csv(
    ...
    sub_score = result[1],
    train_ration = 0,
    csv_file_path="rps.csv"
)

# Epoch 123/1000
# 걸린시간 :  309.465 초
# loss :  0.00119670364074409
# accuracy :  1.0